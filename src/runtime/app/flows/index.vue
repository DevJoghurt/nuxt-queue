              ...
              <div class="flex items-center justify-between">
                <div>
                  ...
                  <div class="text-xs text-gray-500">
                    {{ new Date(r.ts || r.startedAt || Date.now()).toLocaleString() }}
                  </div>
                </div>
                <div>
                  <UButton
...
const refreshRuns = async () => {
  ...
  const res = await $fetch<any[]>(`/api/_queue/flows/${encodeURIComponent(selectedFlow.value)}/runs`)
  runs.value = res || []
}
...
const pageTimeline = async () => {
  ...
  try {
    ...
    const url = `/api/_queue/flows/${encodeURIComponent(selectedFlow.value)}/runs/${encodeURIComponent(timelineRunId.value)}/timeline?limit=${limit}${lastTimelineId.value ? `&fromId=${encodeURIComponent(lastTimelineId.value)}` : ''}`
    const recs = await $fetch<any[]>(url)
    if (recs && recs.length) {
      timeline.value.push(...recs)
      lastTimelineId.value = recs[recs.length - 1].id
    }
  }
  finally {
...
const toggleTail = async () => {
  ...
  if (!selectedFlow.value || !timelineRunId.value) return
  const url = `/api/_queue/flows/${encodeURIComponent(selectedFlow.value)}/runs/${encodeURIComponent(timelineRunId.value)}/tail`
  startSSE(url, (msg) => {
    if (msg?.record) {
...
onMounted(async () => {
  flows.value = await $fetch<any[]>('/api/_queue/flows')
})
</script>
