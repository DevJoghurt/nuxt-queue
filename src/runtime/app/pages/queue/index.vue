    ...
    <div>
      ...
      <div class="space-y-4">
        <QueueListItem
          ...
          :title="queue.name"
          :origin="queue.origin"
          :link="`/queue/${encodeURIComponent(queue.name)}/jobs`"
          :dropdown="[
            [{
...
const {
  data: queues,
  refresh,
} = await useFetch('/api/_queues', {
  method: 'GET',
})

// Metrics map: { [queueName]: { counts?: {..}, paused?: boolean } }
const { data: metrics, refresh: refreshMetrics } = await useFetch('/api/_queues/metrics', { method: 'GET' })

const togglePause = async (name: string, currentlyPaused: boolean) => {
  const action = currentlyPaused ? 'resume' : 'pause'
  await $fetch(`/api/_queues/${name}/${action}`, { method: 'POST' })
